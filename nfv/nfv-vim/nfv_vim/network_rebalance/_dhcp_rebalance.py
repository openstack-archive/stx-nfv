...
class DHCPAgentRebalance(object):
    ...
    def find_network_with_physical_networks(self,
        ...
        for network_id in agent_networks:
            ...
            DLOG.debug("network_id: %s physnet:%s, physical_networks:%s" % (network_id, physnet, physical_networks))
        ...
        if network_match:
            ...
            # we couldn't find a network with networks matching the requirements
            return None
    ...
    def move_agent_network_to_cant_schedule(self, network_to_move, agent_index):
        ...
        self.network_ids_per_agent_cant_schedule[source_agent_id].append(network_to_move)
    ...
    def networks_are_balanced(self):
        ...
        if ((max_networks - min_networks) <= _DHCPRebalance.network_diff_threshold):
            DLOG.info("DHCP Agent networks balanced, max:%s min:%s threshold:%s" %
    ...
    def debug_dump(self):
        ...
        DLOG.debug("_DHCPRebalance.network_ids_per_agent= %s" % _DHCPRebalance.network_ids_per_agent)
        DLOG.debug("_DHCPRebalance.physnet_per_network= %s" % _DHCPRebalance.physnet_per_network)
        DLOG.debug("_DHCPRebalance.state_machine_in_progress= %s" % _DHCPRebalance.state_machine_in_progress)
        DLOG.debug("_DHCPRebalance.dhcpagent_idx= %s" % _DHCPRebalance.dhcpagent_idx)
        DLOG.debug("_DHCPRebalance.num_dhcp_agents= %s" % _DHCPRebalance.num_dhcp_agents)
        DLOG.debug("_DHCPRebalance.num_networks_on_agents= %s" % _DHCPRebalance.num_networks_on_agents)
...
def _remove_network_from_dhcp_agent_callback_body(to_agent_id, network_id, response):
    ...
    if response['completed']:
        ...
        # to target agent.
        nfvi.nfvi_add_network_to_dhcp_agent(to_agent_id, network_id, _add_network_to_dhcp_agent_callback())
    else:
...
def _get_dhcp_agent_networks_callback_body(agent_id, response):
    ...
    if response['completed']:
        ...
        for network in result_data:
            _DHCPRebalance.add_network_to_agent(agent_id, network['id'], network['provider:physical_network'])

        DLOG.debug("_DHCPRebalance.dhcpagent_idx = %s, _DHCPRebalance.num_dhcp_agents = %s" %
...
def _get_networks_on_agents():
    """
    Get Networks hosted by an L3 Agent
    ...
    nfvi.nfvi_get_dhcp_agent_networks(agent_id, _get_dhcp_agent_networks_callback(agent_id))
...
def _reschedule_down_agent():
    ...
    while not found_network_to_move:
        ...
        while len(possible_agent_targets) > 0:
            ...
            agent_with_least_networks = possible_agent_targets[agent_with_least_networks_index]

            host_physical_networks = _DHCPRebalance.get_host_physical_networks(agent_with_least_networks)
        ...
        if len(possible_agent_targets) == 0:
            ...
            DLOG.debug("Unable to reschedule network, no valid target found")
            found_network_to_move = False
        else:
            found_network_to_move = True
            _DHCPRebalance.move_agent_network(network_to_move, 0, agent_with_least_networks)
            ...
            target_agent_id = _DHCPRebalance.get_agent_id_from_index(agent_with_least_networks)
            DLOG.debug("Rescheduling network:%s to agent: %s" % (network_to_move, target_agent_id))
            nfvi.nfvi_remove_network_from_dhcp_agent(source_agent_id,
                                                     network_to_move,
...
def _reschedule_new_agent():
    ...
    Try to achieve a balance of networks hosted by the L3 agents.
    """
    global _DHCPRebalance
    ...
    agent_with_most_networks_index = possible_agent_targets.index(agent_with_most_networks)
    ...
    while (True):
        ...
        if network_to_move is None:
            ...
            # as a candidate.
            DLOG.debug("Could not find a network to move to agent %s" % agent_with_least_networks)
            agent_with_least_networks_index = possible_agent_targets.index(agent_with_least_networks)
            possible_agent_targets.pop(agent_with_least_networks_index)
            num_networks_on_agents.pop(agent_with_least_networks_index)
            ...
            if len(possible_agent_targets) == 0:
                ...
                DLOG.debug("No more agents to try for max network agent")

                _DHCPRebalance.agent_list_remove((agent_with_most_networks, max_networks))
            ...
            # before we move this network, it is possible that due to incompatible networks,
            # we now are looking at an agent that doesn't meet our threshold requirements
            # if that is the case, do not move the network.  We are done trying to move
            # networks off this agent
            if (max_networks - min_networks) <= _DHCPRebalance.network_diff_threshold:
                DLOG.debug("No more agents to try for max network agent "
            ...
            source_agent_id = _DHCPRebalance.get_agent_id_from_index(agent_with_most_networks)
            target_agent_id = _DHCPRebalance.get_agent_id_from_index(agent_with_least_networks)
...
def _run_state_machine():
    """
    Network Rebalance state machine
    """
    global _DHCPRebalance
...
def dr_initialize():
    ...
    if config.section_exists('dhcp-agent-rebalance'):
        ...
        _DHCPRebalance.network_diff_threshold = int(section.get('network_diff_threshold', 3))
