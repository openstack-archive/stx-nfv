...
class NeutronExtensionName(Constants):
    """
...
EXTENSION_NAME = NeutronExtensionName()
...
def delete_network_agents(token, host_name):
    ...
    for agent in AGENT_TYPE.__iter__():
    ...
    if num_agents_found != num_agents:
...
def enable_network_agents(token, host_name):
    ...
    # Return last response with aggregated admin_state_up.
    response.result_data['agent']['admin_state_up'] = all_enabled
    return response
...
def disable_network_agents(token, host_name):
    ...
    # Return last response with aggregated admin_state_up.
    response.result_data['agent']['admin_state_up'] = not all_disabled
    return response
...
def query_host_services(token, host_name):
    ...
    api_cmd_headers['wrs-header'] = 'true'
    api_cmd_headers['Content-Type'] = "application/json"
    api_cmd_headers['host'] = host_name
...
def query_network_agents(token, host_name, check_fully_up):
    ...
    response = rest_api_request(token, "GET", api_cmd, api_cmd_headers)
    if 0 == len(response.result_data['agents']):
        return None

    agents_present = response.result_data.get('agents', list())
            ...
            if found:
                if check_fully_up:
                    if not (alive and admin_state_up):
                        DLOG.warn("host %s agent %s not fully up. alive: %s,"
                                  " admin_state_up: %s" %
                    ...
                    if not alive:
                        DLOG.warn("host %s agent %s not alive" %
                                  (host_name, alive))
                        agent_state = 'down'
                        break
            else:
