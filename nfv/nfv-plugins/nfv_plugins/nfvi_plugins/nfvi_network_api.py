    ...
    def create_host_services(self, future, host_uuid, host_name,
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if neutron.lookup_extension(neutron.EXTENSION_NAME.HOST,
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if self._host_supports_kubernetes():
                    extension_name = neutron.EXTENSION_NAME.AGENT
                else:
                    extension_name = neutron.EXTENSION_NAME.HOST
                ...
                if neutron.lookup_extension(extension_name,
                    ...
                    if not self._host_supports_kubernetes():
                        ...
                        if future.result.data is not None:
                            result_data = future.result.data.get('agent', dict())
                            if not (result_data and
                                    result_data.get('admin_state_up', False) is True and
                                    host_name == result_data.get('host', '')):
                ...
                if neutron.lookup_extension(extension_name,
                    ...
                    if not self._host_supports_kubernetes():
                        ...
                        if future.result.data is not None:
                            result_data = future.result.data.get('agent', dict())
                            if not (result_data and
                                    result_data.get('admin_state_up', True) is False and
                                    host_name == result_data.get('host', '')):
